...
@register("astrbot_plugin_bili_cookie_monitor", "fimore", "B站Cookie状态监控插件(扫码登录增强版)", "3.1.2", "https://github.com/fimore/astrbot_plugin_bili_cookie_monitor")
class BiliCookieMonitorPlugin(Star):
    ...
    async def terminate(self):
        ...
        # 取消所有扫码轮询任务
        for uid, task in list(self._login_tasks.items()):
            if not task.done():
                task.cancel()
